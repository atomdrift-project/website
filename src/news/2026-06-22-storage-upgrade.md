...
We are growing the disks under the PostgreSQL master and every replica to hold a dataset that has crossed **4TB** — the record of every analysis the lab has ever run, now spanning **more than 41 million files**. That number crept up on us: each release [forager](https://codeberg.org/atomdrift/forager) pulls from the registries gets parsed, scored, and filed, and the corpus only grows. The old volumes were sized for last quarter's ingest rate, not today's.
