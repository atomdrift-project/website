...
**Pure-Rust inference.** The XGBoost and LightGBM loaders are gone. Scan now runs ONNX-only against the new [azoth](/azoth/) model — one inference path, no native ML runtime to fight with. It sits on the rebuilt analysis engine: [cleave](/cleave/) 2.0 and [filefacts](https://codeberg.org/atomdrift/filefacts) feed it richer package, PE/CLR, archive, and string signals than 1.x ever saw.
...
[v2.0.0 release notes](https://codeberg.org/atomdrift/scan/releases/tag/v2.0.0).
